# Project Initialization Configuration - Phoenix SoC v1.0.0
# CBFlow initialization configuration for the Phoenix project
...
array set project {
    name "phoenix"
    version "v1.0.0"
    description "Phoenix SoC - High Performance Application Processor"
    design_top "phoenix_soc"
}
...
array set flow_settings {
    release_version $::env(FLOW_RELEASE_VERSION)
...
# 5. CBFlow root directory (dynamically resolved, portable)
array set cbflow {
...
if {[file exists "$cbflow(root)/utils/utilities/$_utils_ver/utils.tcl"]} {
...
puts "✅ CBFlow initialization config loaded: $project(name) ($technology(vendor)_$technology(node)) - $flow_settings(release_version)"
