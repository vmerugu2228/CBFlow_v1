# Project Initialization Configuration - Denali v1.0.0
# CBFlow initialization configuration for the Denali project
...
array set project {
    name "denali"
    version "v1.0.0"
    description "Denali chip design project"
    design_top "denali_top"
}
...
if {[info exists argv0] && [file tail $argv0] eq [file tail [info script]]} {
    ...
    if {$cbflow(root) ne $cbflow_root_auto} {
        puts "WARNING: Config cbflow(root): $cbflow(root)"
        puts "WARNING: Auto-detected root: $cbflow_root_auto"
    ...
    if {[catch {
        ...
        puts "ERROR: Failed to generate shell environment file: $error"
        exit 1
    }
    ...
    if {[catch {
        ...
        puts "ERROR: Failed to generate TCL environment file: $error"
        exit 1
    }
