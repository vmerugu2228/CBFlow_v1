...
# Merge metadata
array set lec {
    stages {inputs1 setup1 compare1 analyze1}

    merge_entry_stage     inputs1
    merge_handoff_stage   analyze1
    merge_parallel_stages {}

    subnodes,inputs1 {setup netlist_golden netlist_revised constraints validate finish}
...
array set lec {
    ...
    subnode_dependencies,inputs1,netlist_golden {setup}
    subnode_dependencies,inputs1,netlist_revised {setup}
    subnode_dependencies,inputs1,constraints {setup}
    subnode_dependencies,inputs1,validate {netlist_golden netlist_revised constraints}
...
array set lec {
    ...
    node_descriptions,inputs1 "Input file validation and preparation for netlist comparison (6 subnodes: setup, netlist_golden, netlist_revised, constraints, validate, finish)"
...
# ┌─ File Requirements ───────────────────────────────────────────────────────┐
array set lec {
    critical_files,inputs1 {lec(input,netlist_golden) lec(input,netlist_revised)}
    ...
    optional_files,inputs1 {lec(input,constraints)}
}
...
if {![info exists ::lec_config_loaded]} {
    puts "INFO: LEC configuration loaded successfully - [llength $lec(stages)] stages, [expr {[llength $lec(subnodes,inputs1)] + [llength $lec(subnodes,setup1)] + [llength $lec(subnodes,compare1)] + [llength $lec(subnodes,analyze1)]}] total subnodes"
