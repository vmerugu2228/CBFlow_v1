...
if {$current_flow_type eq ""} {
    ...
    if {[info exists argc] && [info exists argv] && $argc > 0} {
        ...
        if {!$load_all_flows} {
            foreach arg $argv {
                if {[regexp {^(SYNTH|FP|PNR|STA|LEC|EMIR|PV|ECO|CLP|POPT|FCFP|SYNTH_PNR)$} [string toupper $arg]]} {
...
# Note: Notification settings and version information are now in logo_config.tcl
...
# ═══════════════════════════════════════════════════════════════════════════════# CBflow v1.0.2 — tool launch chain + LSF+xterm support
