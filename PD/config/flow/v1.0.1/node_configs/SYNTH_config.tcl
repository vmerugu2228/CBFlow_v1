...
array set synth {
    stages {inputs1 mmmc_setup1 synthesis1 export_data1 release_data1}

    merge_entry_stage     inputs1
    merge_handoff_stage   export_data1
    merge_parallel_stages {release_data1}

    subnodes,inputs1       {rtl sdc library upf validate finish}
...
array set synth {
    ...
    dependencies,synthesis1    {mmmc_setup1}
...
array set synth {
    ...
    subnode_dependencies,inputs1,validate {rtl sdc library upf}
...
if {![info exists ::synth_config_loaded]} {
    puts "INFO: SYNTH configuration loaded successfully - [llength $synth(stages)] stages, [expr {[llength $synth(subnodes,inputs1)] + [llength $synth(subnodes,synthesis1)] + [llength $synth(subnodes,export_data1)] + [llength $synth(subnodes,release_data1)]}] total subnodes"
