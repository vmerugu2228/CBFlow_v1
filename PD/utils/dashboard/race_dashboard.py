...
class DashboardHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _action_save_node_full_config(self, body):
        ...
        # Determine which override file(s) to write
        targets = []
        if scope == 'branch' and branch_key:
            # Write to all nodes in this branch
            rtf = os.path.join(setup_dir, 'runtime_flow_config.tcl')
            if os.path.exists(rtf):
                with open(rtf) as f:
                    content = f.read()
                for m in re.finditer(r'stages,(\w+),branch_key\s+' + re.escape(branch_key), content):
                    targets.append(m.group(1))
            if not targets:
                targets = [stage]
        elif scope == 'type':
            # Write to base type override (applies to all nodes of this type)
            targets = [stage_base]
        else:
            targets = [stage]

        # Build override content
        lines = [f'# Override config for {", ".join(targets)} ({self.dashboard.flow_type})']
        ...
        # Write override file(s)
        written = []
        for target in targets:
            target_base = re.sub(r'\d+(_\w+)?$', '', target)
            fname = f'override_config.{target_base}.tcl'
