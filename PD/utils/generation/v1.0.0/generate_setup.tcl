...
namespace eval ::CBFlow::Generation::SetupGenerator {
    ...
    proc get_hierarchical_config_paths {flow_dir run_dir flow_type node_type node_name} {
        ...
        if {$project_name ne ""} {
            lappend config_files [list "cbflow_init_config" "$config_root/project/$project_name/$project_version/cbflow_init_config.tcl"]
        ...
        # 2. Metal Stack Configuration (must be sourced BEFORE tech_config — provides tech(metal_count), routing layers)
        if {$tech_name ne ""} {
            # Read project(metal_stack) from already-sourced project_config
            set _ms_name ""
            if {[info exists project(metal_stack)]} { set _ms_name $project(metal_stack) }
            if {$_ms_name ne ""} {
                set _ms_path "$config_root/tech/$tech_name/$tech_version/metal_stack/${_ms_name}.tcl"
                if {[file exists $_ms_path]} {
                    lappend config_files [list "metal_stack_config" "$_ms_path"]
                }
            }
        }

        # 3. Technology Configuration (pure data — uses project(lib_root) and tech(metal_count) from above)
        if {$tech_name ne ""} {
        ...
        if {$tech_name ne ""} {
            ...
            if {[info exists project(lib_config_version)] && $project(lib_config_version) ne ""} {
                set _lib_ver $project(lib_config_version)
            }
            if {$_lib_ver ne ""} {
                set _lc "$config_root/tech/$tech_name/$tech_version/lib_config_${_lib_ver}.tcl"
            } else {
                set _lc "$config_root/tech/$tech_name/$tech_version/lib_config.tcl"
            }
            if {[file exists $_lc]} {
    ...
    proc generate_setup_file {flow_dir run_dir flow_type node_type node_name} {
        ...
        lappend lines "} else {"
        lappend lines "    puts \"ERROR: Command file not found: \$tool_cmd_path\""
        lappend lines "}"
        lappend lines ""
