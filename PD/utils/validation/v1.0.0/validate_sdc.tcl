#!/usr/bin/env tclsh
# ═══════════════════════════════════════════════════════════════════════════════
# OMNI FLOW - SDC Validation Script  
# Description: Validate SDC constraint files for PNR flow
# Usage: tclsh validate_sdc.tcl <run_dir>
# ═══════════════════════════════════════════════════════════════════════════════
...
if {$argc < 1} {
    ...
    handle_error "Usage: tclsh validate_sdc.tcl <run_dir>"
}
...
# Determine flow type from environment or default to PNR for SDC validation
set flow_type "PNR"
...
handle_info "Validating SDC files using $handler"
set result [catch {exec tclsh $handler "sdc" $run_dir} err]
if {$result != 0} {
    handle_error "SDC validation failed: $err"
} else {
    handle_info "SDC validation completed successfully."
}