...
# ── Head-block manifest (bypass-safe load_design) ──────────────────────────
# A tiny Tcl-sourceable text file at <run_dir>/.cbflow_head_block.tcl tracks
# the most-recently-saved FC block in the design lib. Each FC PnR stage's
# save_design calls cbflow_set_head_block after its `save_block -as`; the
# next stage's load_design calls cbflow_get_head_block to find what block to
# copy from. When a stage is BYPASSED, race_engine.py:bypass updates only
# the DB status — the manifest is NOT touched, so the pointer naturally
# remains at the last stage that actually saved. The next downstream stage
# transparently picks up that block. Zero FC invocations during bypass.

proc cbflow_set_head_block {block_name stage_name {node_name ""}} {
    set _f "$::env(CBFLOW_RUN_DIR)/.cbflow_head_block.tcl"
    set _ts [clock format [clock seconds] -format {%Y-%m-%dT%H:%M:%S}]
    set fh [open $_f w]
    puts $fh "# Auto-generated by CBflow — last successfully saved FC block."
    puts $fh "set ::CBFLOW_HEAD_BLOCK     \"$block_name\""
    puts $fh "set ::CBFLOW_HEAD_STAGE     \"$stage_name\""
    puts $fh "set ::CBFLOW_HEAD_NODE      \"$node_name\""
    puts $fh "set ::CBFLOW_HEAD_TIMESTAMP \"$_ts\""
    close $fh
}

# Reader — returns the head block name, or $default if no manifest exists
# yet (first-stage run, fresh workarea, or manifest deleted manually).
proc cbflow_get_head_block {default} {
    set _f "$::env(CBFLOW_RUN_DIR)/.cbflow_head_block.tcl"
    if {[file exists $_f]} {
        source $_f
        if {[info exists ::CBFLOW_HEAD_BLOCK] && $::CBFLOW_HEAD_BLOCK ne ""} {
            return $::CBFLOW_HEAD_BLOCK
        }
    }
    return $default
}
