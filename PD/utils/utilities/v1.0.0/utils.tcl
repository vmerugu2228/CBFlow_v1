...
if {![info exists ::cbflow_utils_loaded]} {
    ...
    proc ::handle_error {message {exit_code 1}} {
        ...
        if {$exit_code != 0} {
            # In test_mode, `exit` is renamed to a no-op printer — calling it
            # leaves the caller running on stale state and Tcl then crashes
...
proc flow_exec {name} {
    ...
    # In test_mode, isolate flow_procs from each other — an abort in one
    # (handle_error → return -code error, or an undefined-var crash) should
    # NOT prevent the next flow_proc from being executed. In production
...
proc load_mmmc_config {} {
    ...
    # Don't exit (test_mode intercepts exit, leaving the caller's
    # `if {![load_mmmc_config]}` with an empty return value → Tcl error
    # "expected boolean value but got"). Return false so the caller's
    # short-circuit `return` runs cleanly.
    puts "WARNING: MMMC config not found at expected path: $mmmc_path"
    return false
}
...
proc cbflow_record_block_state {stage_name block_name {node_name ""}} {
    ...
    # `clock format` walks into ::tcl::clock::GetSystemTimeZone which
    # test_mode's `unknown` interceptor renames away. Wrap in catch so
    # we never crash the cmd file on a cosmetic timestamp.
    if {[catch {clock format [clock seconds] -format {%Y-%m-%dT%H:%M:%S}} _ts]} {
        set _ts "epoch [clock seconds]"
    }
    set fh [open $_f w]
