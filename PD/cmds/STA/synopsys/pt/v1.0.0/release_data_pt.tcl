#!/usr/bin/env tclsh
# CBFlow STA release_data - Synopsys PrimeTime | Package release deliverables
...
set config_file "$run_dir/work/STA/release_data/run/config.tcl"
...
handle_info "Starting STA release_data with Synopsys PrimeTime..."
...
flow_proc setup_release_dirs {
    ...
    set design_name [expr {[info exists pt(common,design_name)] ? $pt(common,design_name) : [expr {[info exists flow(design_name)] ? $flow(design_name) : "sta"}]}]
    ...
    if {[namespace exists ::CBFlow::Release]} {
        ...
        # Fallback: use legacy directory
        if {[info exists pt(common,release_dir)]} {
            set release_base $pt(common,release_dir)
        } else {
...
flow_proc generate_release_output {
    ...
    if {[namespace exists ::CBFlow::Release]} {
        ...
        puts $fh ""
        puts $fh "set release_info(tool)       \"PrimeTime\""
        puts $fh "set release_info(vendor)     \"Synopsys\""
