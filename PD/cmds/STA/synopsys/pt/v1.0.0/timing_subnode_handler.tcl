...
switch $subnode_name {
    ...
    default {
        ...
        set _fh [open $_ctx "w"]
        puts $_fh "# Scenario context for $scenario — auto-generated by subnode handler"
        ...
        # Per-RC-corner SPEF from extraction manifest
        ...
        close $_fh
        puts "INFO: Scenario context written: $_ctx"

        if {$test_mode} {
            puts "INFO: \[TEST MODE\] Scenario $scenario"
            puts "INFO:   Corner=$_sv(corner) Mode=$_sv(mode) Voltage=$_sv(voltage) Temp=$_sv(temperature)"
            puts "INFO:   RC=$_sv(rc_corner) LibSet=$_sv(lib_set_ref)"
            puts "INFO:   SDC=$_sv(constraint_file)"
            # Create report stub
            set rpt [open "$run_dir/reports/sta/timing_${scenario}_summary.rpt" "w"]
            puts $rpt "# Test mode summary for scenario: $scenario"
            puts $rpt "# Corner: $_sv(corner), Mode: $_sv(mode), Voltage: $_sv(voltage), Temp: $_sv(temperature)"
            puts $rpt "Setup WNS: 0.000ns  Hold WNS: 0.000ns"
            close $rpt
            puts "INFO: $stage_name scenario $scenario completed \[TEST MODE\]"
        } else {
            # Scenario-specific command file
            set cmd_file "$::env(FLOW_DIR)/cmds/STA/synopsys/pt/$_tool_ver/timing_scenario_pt.tcl"

            # Build wrapper script (same pattern as "run" subnode)
            set _tool_name [expr {[info exists sta(tool,name)] ? $sta(tool,name) : "pt"}]
            ...
            set _wf [open $_wrapper "w"]
            puts $_wf "#!$_wrapper_shell"
            puts $_wf "# CBFlow STA scenario launch — $scenario"
            puts $_wf "# Generated: [clock format [clock seconds]]"
            ...
            # Launch — use local execution for per-scenario (LSF handled by RACE engine)
            puts "INFO: Launching: $_wrapper"
