#!/usr/bin/env tclsh
# CBFlow STA mmmc_setup - Synopsys PrimeTime | MMMC scenario configuration
...
set config_file "$run_dir/work/STA/mmmc_setup/run/config.tcl"
...
flow_proc generate_dynamic_makefiles {
    ...
    foreach scenario $mmmc_active_scenarios {
        ...
        puts $fh "# CBFlow STA Per-Scenario Timing Makefile"
        ...
        set stamp_var "TIMING_[string toupper $safe_name]_STAMP"

        puts $fh "# Scenario: $scenario"
        puts $fh "$stamp_var = \$(STAMP_DIR)/timing_${scenario}.stamp"
        puts $fh ""
        puts $fh ".PHONY: timing_${scenario}"
        puts $fh "timing_${scenario}: \$($stamp_var)"
        puts $fh ""
        puts $fh "\$($stamp_var): \$(STAMP_DIR)/extraction1.stamp"
        puts $fh "\t@echo \"Running timing for scenario: ${scenario}...\""
        puts $fh "\t@mkdir -p \$(STAMP_DIR) \$(LOGS_DIR)"
        puts $fh "\t@tclsh \"\$(FLOW_DIR)/cmds/STA/synopsys/pt/\$(PT_VERSION)/timing_scenario_handler.tcl\" ${scenario} \"\$(PWD)\""
    ...
    set scenario_stamps {}
    foreach scenario $mmmc_active_scenarios {
        lappend scenario_stamps "\$(STAMP_DIR)/timing_${scenario}.stamp"
    }

    puts $fh "# Aggregate timing1 target - depends on all per-scenario stamps"
    puts $fh ".PHONY: timing1"
    puts $fh "timing1: \$(STAMP_DIR)/timing1.stamp"
    puts $fh ""
    puts $fh "\$(STAMP_DIR)/timing1.stamp: [join $scenario_stamps { }]"
