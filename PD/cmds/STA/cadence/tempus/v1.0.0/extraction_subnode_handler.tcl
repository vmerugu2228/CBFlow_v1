#!/usr/bin/env tclsh
# ═══════════════════════════════════════════════════════════════════════════════
# CBFlow STA EXTRACTION Subnode Handler
# Description: Handles extraction stage subnodes (setup, run, validate, finish)
...
set cmd_file "$::env(FLOW_DIR)/cmds/STA/cadence/tempus/[expr {[info exists ::env(TEMPUS_VERSION)] ? $::env(TEMPUS_VERSION) : "v1.0.0"}]/extraction_tempus.tcl"
...
switch $subnode_name {
    ...
    default {
        ...
        if {[string match "rc_*" $subnode_name]} {
            ...
            # Resolve QRC tech file from tech_config: tech(rcx,<corner>,qrc)
            # Priority: qrc → nxtgrd → tluplus
            set _rc_file ""
            set _rc_file_type ""
            set _rc_corner $subnode_name

            if {[info exists tech(rcx,${_rc_corner},qrc)] && $tech(rcx,${_rc_corner},qrc) ne ""} {
                set _rc_file $tech(rcx,${_rc_corner},qrc)
                set _rc_file_type "qrc"
            }
            if {$_rc_file eq "" && [info exists tech(rcx,${_rc_corner},nxtgrd)] && $tech(rcx,${_rc_corner},nxtgrd) ne ""} {
            ...
            if {$test_mode} {
                ...
                # Generate Quantus/QRC command file and invoke
