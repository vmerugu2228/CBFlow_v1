...
# Based on eco(eco,type): "timing" uses eco_opt, "functional" uses eco_netlist
...
set config_file "$run_dir/work/ECO/eco1/run/config.tcl"
...
set ::eco_type "timing"
if {[info exists eco(eco,type)]} { set ::eco_type $eco(eco,type) }
...
flow_proc load_design {
    ...
    set design_name [expr {[info exists eco(design_name)] ? $eco(design_name) : $flow(design_name)}]
    set lib_name [expr {[info exists eco(design_lib_name)] ? $eco(design_lib_name) : "${design_name}.nlib"}]
...
flow_proc configure_eco {
    ...
    if {[info exists eco(non_persistent_script)] && [file exists $eco(non_persistent_script)]} {
        source -e $eco(non_persistent_script)
    }

    # FC-RM: User pre-ECO script
    if {[info exists eco(eco_pre_script)] && [file exists $eco(eco_pre_script)]} {
        source -e $eco(eco_pre_script)
    }
...
flow_proc post_eco {
    ...
    if {[info exists eco(eco_post_script)] && [file exists $eco(eco_post_script)]} {
        source -e $eco(eco_post_script)
    }
...
# ==============================================================================
set _setup_file "$run_dir/work/ECO/eco1/run/setup.tcl"
