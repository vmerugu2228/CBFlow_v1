#!/usr/bin/env tclsh
# ═══════════════════════════════════════════════════════════════════════════════
# CBFlow ECO ECO Subnode Handler
# Description: Handles eco stage subnodes (setup, run, validate, finish)
...
set cmd_file "$::env(FLOW_DIR)/cmds/ECO/synopsys/icc2/[expr {[info exists ::env(ICC2_VERSION)] ? $::env(ICC2_VERSION) : "v1.0.0"}]/eco_icc2.tcl"
...
switch $subnode_name {
    ...
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists eco(tool,name)] ? $eco(tool,name) : "icc2"}]
            set _log_file "$_work_dir/${node_name}.log"
            set _module_cmd ""
            set _tool_shell "icc2_shell"
            catch {
