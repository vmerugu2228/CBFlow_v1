...
flow_proc prepare_release {
    ...
    set missing_vars {}
    if {![info exists pnr(design_name)] && ![info exists flow(design_name)]} {
        lappend missing_vars "design_name (pnr(design_name) or flow(design_name))"
    }
    ...
    if {[info exists pnr(release_phase)]} { set release_phase $pnr(release_phase) }
    ...
    # Copy GDS
    set gds_src "$run_dir/results/pnr/gds/$pnr(design_name).gds2"
    if {[file exists $gds_src]} {
        file copy -force $gds_src "$release_dir/gds/$pnr(design_name).gds2"
    ...
    foreach suffix [list "" "_pg"] {
        set nl_src "$run_dir/results/pnr/netlist/$pnr(design_name)${suffix}.v"
        if {$suffix eq "_pg"} {
            set nl_dst "$release_dir/netlist/pg/$pnr(design_name)_pg.v"
        } else {
            set nl_dst "$release_dir/netlist/$pnr(design_name).v"
        }
    ...
    # Copy SDF
    set sdf_src "$run_dir/results/pnr/netlist/$pnr(design_name).sdf"
    if {[file exists $sdf_src]} {
        file copy -force $sdf_src "$release_dir/sdf/$pnr(design_name).sdf"
        handle_info "Copied SDF to release"
    }

    # Copy DEF
    set def_src "$run_dir/results/pnr/def/$pnr(design_name).def"
    if {[file exists $def_src]} {
        file copy -force $def_src "$release_dir/def/$pnr(design_name).def"
        handle_info "Copied DEF to release"
    }

    # Copy SDC
    set sdc_src "$run_dir/results/pnr/sdc/$pnr(design_name).sdc"
    if {[file exists $sdc_src]} {
        file copy -force $sdc_src "$release_dir/sdc/$pnr(design_name).sdc"
        handle_info "Copied SDC to release"
    }

    # Copy SPEF files (all corners)
    foreach corner {max min nom} {
        set spef_src "$run_dir/results/pnr/spef/$pnr(design_name)_${corner}.spef"
        if {[file exists $spef_src]} {
            file copy -force $spef_src "$release_dir/spef/$pnr(design_name)_${corner}.spef"
...
flow_proc generate_manifest {
    ...
    puts $fh "# Design: $pnr(design_name)"
...
flow_proc validate_release {
    ...
    set required_files [list \
        "gds/$pnr(design_name).gds2" \
        "netlist/$pnr(design_name).v" \
        "netlist/pg/$pnr(design_name)_pg.v" \
        "def/$pnr(design_name).def" \
        "sdc/$pnr(design_name).sdc" \
        "spef/$pnr(design_name)_max.spef" \
        "spef/$pnr(design_name)_min.spef" \
        "MANIFEST.txt" \
    ]
    ...
    if {$release_pass} {
        ...
        puts $stamp_fh "Design: $pnr(design_name)"
