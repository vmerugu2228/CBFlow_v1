...
set config_file "$run_dir/work/PNR/export_data1/run/config.tcl"
...
flow_proc write_netlist {
    ...
    set netlist_logic "$run_dir/results/pnr/netlist/$pnr(design_name).v"
    ...
    set netlist_lvs "$run_dir/results/pnr/netlist/$pnr(design_name).lvs.v"
    ...
    set netlist_pt "$run_dir/results/pnr/netlist/$pnr(design_name).pt.v"
    ...
    set netlist_fm "$run_dir/results/pnr/netlist/$pnr(design_name).fm.v"
...
flow_proc write_def_output {
    ...
    set def_file "$run_dir/results/pnr/def/$pnr(design_name).def"
...
flow_proc write_gds_output {
    ...
    if {[info exists pnr(output,write_gds)] && $pnr(output,write_gds)} {
        set gds_file "$run_dir/results/pnr/gds/$pnr(design_name).gds"
    ...
    if {[info exists pnr(output,write_oasis)] && $pnr(output,write_oasis)} {
        set oasis_file "$run_dir/results/pnr/gds/$pnr(design_name).oasis"
...
flow_proc write_parasitics_output {
    ...
    set spef_output "$run_dir/results/pnr/spef/$pnr(design_name)"
...
flow_proc write_sdc_output {
    ...
    write_script -force -compress gzip \
        -output "$run_dir/results/pnr/sdc/${pnr(design_name)}_wscript"
    write_script -force -compress gzip -format pt \
        -output "$run_dir/results/pnr/sdc/${pnr(design_name)}_wscript_for_pt"

    # Write routing constraints
    write_routing_constraints \
        "$run_dir/results/pnr/sdc/${pnr(design_name)}_routing_constraints"
    ...
    write_floorplan \
        ...
        -output "$run_dir/results/pnr/sdc/${pnr(design_name)}_floorplan"

    # Write UPF if power intent exists
    if {[info exists pnr(output,write_upf)] && $pnr(output,write_upf)} {
        save_upf "$run_dir/results/pnr/sdc/$pnr(design_name).upf"
    ...
    foreach_in_collection scn [all_scenarios] {
        ...
        write_sdc -compress gzip \
            -output "$run_dir/results/pnr/sdc/${pnr(design_name)}_${scnN}.sdc"
    }
...
flow_proc save_design_block {
    ...
    if {[info exists pnr(output,block_labeling)] && $pnr(output,block_labeling)} {
        save_block -as $pnr(design_name)/export_data
        handle_info "Block saved as $pnr(design_name)/export_data"
    } else {
        save_block
...
# ==============================================================================
set _setup_file "$run_dir/work/PNR/export_data1/run/setup.tcl"
