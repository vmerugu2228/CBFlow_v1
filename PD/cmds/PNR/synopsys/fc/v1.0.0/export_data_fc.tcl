#!/usr/bin/env tclsh
# CBFlow shared (PNR + SYNTH_PNR) — synopsys/fc — export_data (write_data) - Synopsys Fusion Compiler
...
flow_proc load_design {
    ...
    if {[info exists cfg(export_data,pre_script)] && [file exists $cfg(export_data,pre_script)]} {
        source $cfg(export_data,pre_script)
    }
...
flow_proc change_names {
    ...
    if {[info exists cfg(export_data,name_rules)] && $cfg(export_data,name_rules) ne ""} {
        eval define_name_rules verilog $cfg(export_data,name_rules)
    }
...
flow_proc write_gds_output {
    ...
    if {[info exists cfg(export_data,write_oasis)] && $cfg(export_data,write_oasis) ne "" && [string is true -strict $cfg(export_data,write_oasis)]} {
...
flow_proc write_def_output {
    ...
    if {[info exists cfg(export_data,def_convert_sites)] && $cfg(export_data,def_convert_sites) ne ""} {
        lappend def_cmd -convert_sites $cfg(export_data,def_convert_sites)
    }
...
flow_proc write_upf_output {
    ...
    set upf_mode [expr {$cfg(common,upf_mode) ne "" ? $cfg(common,upf_mode) : "none"}]
...
flow_proc save_design {
    ...
    save_block
    if {[info exists cfg(export_data,block_labeling)] && $cfg(export_data,block_labeling) ne "" && [string is true -strict $cfg(export_data,block_labeling)]} {
        save_block -as ${design_name}/write_data
        handle_info "Block saved: ${design_name}/write_data"
    }

    # FC-RM: User post-write_data script
    if {[info exists cfg(export_data,post_script)] && [file exists $cfg(export_data,post_script)]} {
        source $cfg(export_data,post_script)
    }
