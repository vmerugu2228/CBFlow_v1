...
set node_config "$::env(CONFIG_ROOT)/flow/$::env(FLOW_CONFIG_VERSION)/node_configs/PNR_config.tcl"
...
switch $subnode_name {
    "netlist" {
        puts "INFO: PNR $stage_name netlist..."
        if {$test_mode} { puts "INFO: \[TEST MODE\] netlist loading skipped" }
        set ff "$run_dir/work/PNR/$node_name/netlist/netlist_info.tcl"
    ...
    "library" {
        ...
        puts "INFO: PNR $stage_name library completed"
    }
    "validate" {
        puts "INFO: PNR $stage_name validate..."
        if {$test_mode} { puts "INFO: \[TEST MODE\] Validation skipped" }
        puts "INFO: PNR $stage_name validate completed"
    }
    "finish" {
        puts "INFO: PNR $stage_name finish..."
        set ff "$run_dir/work/PNR/$node_name/finish/finish_info.tcl"
        file mkdir [file dirname $ff]
        set fh [open $ff "w"]; puts $fh "set finish_info(timestamp) \"[clock format [clock seconds]]\""; close $fh
        puts "INFO: PNR $stage_name finish completed"
    }
