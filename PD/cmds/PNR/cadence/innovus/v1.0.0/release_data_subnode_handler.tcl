#!/usr/bin/env tclsh
# ═══════════════════════════════════════════════════════════════════════════════
# CBFlow PNR RELEASE_DATA Subnode Handler
# Description: Handles release_data stage subnodes (setup, run, validate, finish)
...
# Load flow and PNR configuration
...
set cmd_file "$::env(FLOW_DIR)/cmds/PNR/cadence/innovus/[expr {[info exists ::env(INNOVUS_VERSION)] ? $::env(INNOVUS_VERSION) : "v1.0.0"}]/release_data_handler.tcl"
...
switch $subnode_name {
    "setup" {
        ...
        # Generate consolidated config.tcl and setup.tcl for this stage
        if {[info exists ::env(GENERATION_VERSION)] && [info exists ::env(FLOW_DIR)]} {
            set gen_script "$::env(FLOW_DIR)/utils/generation/$::env(GENERATION_VERSION)/generate_setup.tcl"
            if {[file exists $gen_script]} {
                puts "INFO: Generating config for $stage_name..."
                catch {exec tclsh $gen_script $::flow_type $stage_name ${stage_name}_default $run_dir} gen_result
            }
        }
    ...
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists pnr(tool,name)] ? $pnr(tool,name) : "innovus"}]
            ...
            puts $_wf "$_tool_shell -files $cmd_file -log $_log_file"
            close $_wf
