#!/usr/bin/env tclsh
# CBFlow EMIR THERMAL_ANALYSIS Subnode Handler
...
switch $subnode_name {
    ...
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists emir(tool,name)] ? $emir(tool,name) : "voltus"}]
            set _log_file "$_work_dir/${node_name}.log"
            set _module_cmd ""
            set _tool_shell "voltus"
            catch {
    ...
    "validate" {
        ...
        if {$test_mode} { puts "INFO: \[TEST MODE\] Validation skipped" } else { puts "INFO: $stage_name validation passed" }
    ...
    "finish" {
        ...
        set fh [open $finish_file "w"]; puts $fh "Completed: [clock format [clock seconds]]"; close $fh
