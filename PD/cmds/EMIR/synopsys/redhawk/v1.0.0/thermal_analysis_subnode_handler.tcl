#!/usr/bin/env tclsh
# CBFlow EMIR THERMAL_ANALYSIS Subnode Handler - Synopsys REDHAWK
...
set cmd_file "$::env(FLOW_DIR)/cmds/EMIR/synopsys/redhawk/$_tool_ver/thermal_analysis_thermal_analysis_redhawk.tcl"
set test_mode false
...
switch $subnode_name {
    ...
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists emir(tool,name)] ? $emir(tool,name) : "redhawk"}]
            set _log_file "$_work_dir/${node_name}.log"
            set _module_cmd ""
            set _tool_shell "redhawk"
            catch {
