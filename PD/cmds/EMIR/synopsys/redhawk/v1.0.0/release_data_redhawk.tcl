#!/usr/bin/env tclsh
# ===============================================================================
# CBFlow EMIR release_data - Synopsys RedHawk
...
handle_info "Starting EMIR release_data with Synopsys RedHawk..."
...
flow_proc init_release {
    ...
    set missing_vars {}
    if {![info exists redhawk(common,design_name)] && ![info exists flow(design_name)]} {
        lappend missing_vars "design_name (redhawk(common,design_name) or flow(design_name))"
    }
    ...
    set design_name [expr {[info exists redhawk(common,design_name)] ? $redhawk(common,design_name) : [expr {[info exists flow(design_name)] ? $flow(design_name) : "emir"}]}]

    # ── Determine release phase ──────────────────────────────────────────────
    set release_phase "P0"
    if {[info exists project(release_phase)]} { set release_phase $project(release_phase) }
    if {[info exists redhawk(common,release_phase)]} { set release_phase $redhawk(common,release_phase) }
