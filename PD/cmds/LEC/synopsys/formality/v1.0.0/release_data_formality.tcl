...
flow_proc init_release {
    ...
    set missing_vars {}
    if {![info exists formality(common,design_name)] && ![info exists flow(design_name)]} {
        lappend missing_vars "design_name (formality(common,design_name) or flow(design_name))"
    }
    ...
    set design_name [expr {[info exists formality(common,design_name)] ? $formality(common,design_name) : [expr {[info exists flow(design_name)] ? $flow(design_name) : "lec"}]}]

    # ── Determine release phase ──────────────────────────────────────────────
    set release_phase "P0"
    if {[info exists project(release_phase)]} { set release_phase $project(release_phase) }
    if {[info exists formality(common,release_phase)]} { set release_phase $formality(common,release_phase) }
