#!/usr/bin/env tclsh
# ═══════════════════════════════════════════════════════════════════════════════
# CBFlow - LEC Analyze Command File
...
set config_file "$run_dir/work/LEC/analyze1/run/config.tcl"
...
handle_info "Starting LEC analyze stage with Synopsys Formality..."
...
# ── Directories ──────────────────────────────────────────────────────────────
set WORK_DIR "$run_dir/work/LEC/analyze1"
...
flow_proc analyze_failing_points {
    global lec project tech flow
...
flow_proc generate_analysis_log {
    ...
    puts $fp "  4. analyze - Result analysis and reporting"
    puts $fp ""
...
flow_proc validate_analysis_results {
    ...
    handle_info "Running validation for analyze stage..."
    ...
    if {[file exists $enhanced_validation_script]} {
        if {[catch {exec tclsh $enhanced_validation_script LEC analyze $current_dir $validation_flow_dir} validation_result]} {
...
handle_info "═══════════════════════════════════════════════════════════════════════════════"
handle_info " CBFlow LEC Analyze with Synopsys Formality"
handle_info "═══════════════════════════════════════════════════════════════════════════════"

flow_proc analyze_flow {
    handle_info "Executing LEC analyze flow..."

    flow_exec analyze_failing_points
    ...
    handle_info "LEC analyze completed successfully"
}

if {[info exists argv0] && $argv0 eq [info script]} {
    puts "Direct execution mode - running LEC analyze flow"
    flow_exec analyze_flow
} else {
    puts " CBFlow LEC analyze procedures loaded"
    puts "Available: analyze_failing_points, generate_equivalence_report, generate_analysis_log, generate_summary_report, validate_analysis_results, analyze_flow"
}
exit
