#!/usr/bin/env tclsh
# CBFlow FCFP FC_POST_FLOORPLAN Subnode Handler
...
set ::flow_type "FCFP"
set stage_name "fc_post_floorplan"
if {$node_name eq ""} { set node_name $stage_name }
set _tool_ver [expr {[info exists ::env(INNOVUS_VERSION)] ? $::env(INNOVUS_VERSION) : {v1.0.0}}]
set cmd_file "$::env(FLOW_DIR)/cmds/FCFP/cadence/innovus/$_tool_ver/fc_post_floorplan_innovus.tcl"
...
switch $subnode_name {
    ...
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists fcfp(tool,name)] ? $fcfp(tool,name) : "innovus"}]
