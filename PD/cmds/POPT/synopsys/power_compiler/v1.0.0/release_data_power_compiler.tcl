...
flow_proc init_release {
    ...
    set missing_vars {}
    if {![info exists power_compiler(common,design_name)] && ![info exists flow(design_name)]} {
        lappend missing_vars "design_name (power_compiler(common,design_name) or flow(design_name))"
    }
    ...
    set design_name [expr {[info exists power_compiler(common,design_name)] ? $power_compiler(common,design_name) : [expr {[info exists flow(design_name)] ? $flow(design_name) : "popt"}]}]

    # ── Determine release phase ──────────────────────────────────────────────
    set release_phase "P0"
    if {[info exists project(release_phase)]} { set release_phase $project(release_phase) }
    if {[info exists power_compiler(common,release_phase)]} { set release_phase $power_compiler(common,release_phase) }
