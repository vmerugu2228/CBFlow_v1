#!/usr/bin/env tclsh
# CBFlow POPT MERGE_TIMING Subnode Handler - Synopsys PT
...
set _tv [expr {[info exists ::env(PT_VERSION)] ? $::env(PT_VERSION) : {v1.0.0}}]
set cmd_file "$::env(FLOW_DIR)/cmds/POPT/synopsys/pt/$_tv/merge_timing_pt.tcl"
set test_mode false
if {[info exists flow(test_mode)] && $flow(test_mode) eq "true"} { set test_mode true }
switch $subnode_name {
    "setup" { puts "INFO: $stage_name setup..."; file mkdir "$run_dir/work/$::flow_type/$node_name/run"; file mkdir "$run_dir/work/$::flow_type/$node_name/setup"; puts "INFO: $stage_name setup completed" }
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists popt(tool,name)] ? $popt(tool,name) : "pt"}]
    ...
    "validate" { puts "INFO: $stage_name validate..."; if {$test_mode} { puts "INFO: \[TEST MODE\] Validation skipped" }; puts "INFO: $stage_name validate completed" }
    "finish" { puts "INFO: $stage_name finish..."; set ff "$run_dir/work/$::flow_type/$node_name/run/${stage_name}_finish.timestamp"; file mkdir [file dirname $ff]; set fh [open $ff "w"]; puts $fh "Completed: [clock format [clock seconds]]"; close $fh; puts "INFO: $stage_name finish completed" }
