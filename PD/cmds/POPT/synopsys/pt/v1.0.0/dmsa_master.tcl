...
proc dmsa_generate_worker {scenario_name worker_dir} {
    ...
    if {[file exists $template_file]} {
        ...
        puts $wfh "# Generated: [expr {[catch {clock format [clock seconds] -format {%Y-%m-%d %H:%M:%S}} _ts] ? "epoch [clock seconds]" : $_ts}]"
        puts $wfh ""
...
proc dmsa_report_timing {rpt_dir prefix} {
    ...
    if {$::dmsa_available} {
        ...
        puts $sfh "  Generated: [expr {[catch {clock format [clock seconds] -format {%Y-%m-%d %H:%M:%S}} _ts] ? "epoch [clock seconds]" : $_ts}]"
        puts $sfh "═══════════════════════════════════════════════════════════════"
        puts $sfh ""
