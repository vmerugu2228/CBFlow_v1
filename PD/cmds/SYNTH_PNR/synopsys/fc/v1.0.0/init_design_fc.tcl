...
# BUG FIX #1: Source tech_config — was missing, halting flow (tech() array not populated)
...
flow_proc read_design {
    ...
    set rtl_format [expr {[info exists synth_pnr(input,rtl_format)] ? $synth_pnr(input,rtl_format) : "sverilog"}]
    ...
    if {[info exists synth_pnr(compile,qor_mode)] && $synth_pnr(compile,qor_mode) eq "early_design"} {
...
flow_proc load_floorplan {
    ...
    if {[info exists synth_pnr(switch_connectivity_file)] && [file exists $synth_pnr(switch_connectivity_file)]} {
        source -e $synth_pnr(switch_connectivity_file)
...
flow_proc initialize_floorplan {
    ...
    set util [expr {[info exists synth_pnr(fp,core_utilization)] ? $synth_pnr(fp,core_utilization) : 0.70}]
    set ratio [expr {[info exists synth_pnr(fp,aspect_ratio)] ? $synth_pnr(fp,aspect_ratio) : 1.0}]
    set offset [expr {[info exists synth_pnr(fp,core_offset)] ? $synth_pnr(fp,core_offset) : "5 5 5 5"}]
...
flow_proc insert_physical_cells {
    ...
    if {[info exists tech(cells,well_tap)] && $tech(cells,well_tap) ne ""} {
        ...
        set tap_dist [expr {[info exists synth_pnr(fp,tap_cell_distance)] ? $synth_pnr(fp,tap_cell_distance) : 30}]
    ...
    if {[info exists synth_pnr(spare_cells_file)] && [file exists $synth_pnr(spare_cells_file)]} {
        handle_info "  Sourcing spare cells: $synth_pnr(spare_cells_file)"
        source -e $synth_pnr(spare_cells_file)
    }
...
flow_proc load_constraints {
    ...
    if {[info exists synth_pnr(upf_mode)] && $synth_pnr(upf_mode) eq "golden"} {
    ...
    if {[info exists synth_pnr(input,upf_supplemental)] && [file exists $synth_pnr(input,upf_supplemental)]} {
        handle_info "Loading supplemental UPF: $synth_pnr(input,upf_supplemental)"
...
flow_proc setup_mcmm {
    ...
    if {[info exists synth_pnr(mcmm_setup_file)] && [file exists $synth_pnr(mcmm_setup_file)]} {
        handle_info "Sourcing MCMM setup: $synth_pnr(mcmm_setup_file)"
        source -e $synth_pnr(mcmm_setup_file)
    } else {
    ...
    if {[info exists synth_pnr(init_design,active_scenarios)] && $synth_pnr(init_design,active_scenarios) ne ""} {
        set_scenario_status -active false [get_scenarios -filter active]
        set_scenario_status -active true $synth_pnr(init_design,active_scenarios)
        handle_info "Active scenarios (user override): $synth_pnr(init_design,active_scenarios)"
    ...
    if {[info exists synth_pnr(constraints_setup_file)] && [file exists $synth_pnr(constraints_setup_file)]} {
        handle_info "Sourcing constraints setup: $synth_pnr(constraints_setup_file)"
        source -e $synth_pnr(constraints_setup_file)
    }
...
flow_proc setup_timing_variations {
    ...
    if {[info exists synth_pnr(pocv_setup_file)] && [file exists $synth_pnr(pocv_setup_file)]} {
        handle_info "Sourcing POCV setup: $synth_pnr(pocv_setup_file)"
        source -e $synth_pnr(pocv_setup_file)
    ...
    } elseif {[info exists synth_pnr(aocv_setup_file)] && [file exists $synth_pnr(aocv_setup_file)]} {
        # FC-RM: AOCV setup (mutually exclusive with POCV)
        handle_info "Sourcing AOCV setup: $synth_pnr(aocv_setup_file)"
        source -e $synth_pnr(aocv_setup_file)
...
flow_proc setup_lib_cell_purpose {
    ...
    if {[info exists synth_pnr(lib_cell_purpose_file)] && [file exists $synth_pnr(lib_cell_purpose_file)]} {
        handle_info "Sourcing lib cell purpose: $synth_pnr(lib_cell_purpose_file)"
...
flow_proc setup_clock_ndr {
    ...
    if {[info exists synth_pnr(cts_ndr_file)] && [file exists $synth_pnr(cts_ndr_file)]} {
        handle_info "Sourcing CTS NDR rules: $synth_pnr(cts_ndr_file)"
        source -e $synth_pnr(cts_ndr_file)
...
flow_proc setup_placement_constraints {
    ...
    if {[info exists synth_pnr(placement_constraint_files)] && [llength $synth_pnr(placement_constraint_files)] > 0} {
        foreach file $synth_pnr(placement_constraint_files) {
            if {[file exists $file]} {
    ...
    if {[info exists synth_pnr(additional_floorplan_file)] && [file exists $synth_pnr(additional_floorplan_file)]} {
        source -e $synth_pnr(additional_floorplan_file)
    }
...
flow_proc setup_power_activity {
    ...
    # FC-RM: read_saif
    if {[info exists synth_pnr(saif_file)] && $synth_pnr(saif_file) ne ""} {
        if {[file exists $synth_pnr(saif_file)]} {
            set read_saif_cmd "read_saif $synth_pnr(saif_file)"
            if {[info exists synth_pnr(saif_power_scenario)] && $synth_pnr(saif_power_scenario) ne ""} {
                lappend read_saif_cmd -scenarios $synth_pnr(saif_power_scenario)
            }
            if {[info exists synth_pnr(saif_source_instance)] && $synth_pnr(saif_source_instance) ne ""} {
                lappend read_saif_cmd -strip_path $synth_pnr(saif_source_instance)
            }
            if {[info exists synth_pnr(saif_target_instance)] && $synth_pnr(saif_target_instance) ne ""} {
                lappend read_saif_cmd -path $synth_pnr(saif_target_instance)
            }
    ...
    if {[info exists synth_pnr(compile,qor_metric)]} { set qor_metric $synth_pnr(compile,qor_metric) }
...
flow_proc setup_dft {
    ...
    if {[info exists synth_pnr(dft_ports_file)] && [file exists $synth_pnr(dft_ports_file)]} {
        handle_info "Sourcing DFT ports: $synth_pnr(dft_ports_file)"
        source -e $synth_pnr(dft_ports_file)
    }
...
flow_proc set_qor_strategy_init {
    ...
    set metric [expr {[info exists synth_pnr(compile,qor_metric)] ? $synth_pnr(compile,qor_metric) : "timing"}]
    set mode [expr {[info exists synth_pnr(compile,qor_mode)] ? $synth_pnr(compile,qor_mode) : "balanced"}]
...
flow_proc run_floorplan_checks {
    ...
    if {[info exists synth_pnr(floorplan_rule_script)] && [file exists $synth_pnr(floorplan_rule_script)]} {
        source -e $synth_pnr(floorplan_rule_script)
    }
...
flow_proc save_design {
    ...
    # FC-RM: save_upf
    set upf_mode "none"
    if {[info exists synth_pnr(upf_mode)]} { set upf_mode $synth_pnr(upf_mode) }
...
# ==============================================================================
set _setup_file "$run_dir/work/SYNTH_PNR/init_design1/run/setup.tcl"
