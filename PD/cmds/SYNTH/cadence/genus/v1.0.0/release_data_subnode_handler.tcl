#!/usr/bin/env tclsh
# ═══════════════════════════════════════════════════════════════════════════════
# CBFlow SYNTH RELEASE_DATA Subnode Handler
...
# Load flow and SYNTH configuration
...
set pnr_config "$::env(CONFIG_ROOT)/flow/$::env(FLOW_CONFIG_VERSION)/node_configs/SYNTH_config.tcl"
...
set cmd_file "$::env(FLOW_DIR)/cmds/SYNTH/cadence/genus/[expr {[info exists ::env(GENUS_VERSION)] ? $::env(GENUS_VERSION) : "v1.0.0"}]/release_db_genus.tcl"
...
switch $subnode_name {
    ...
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists synth(tool,name)] ? $synth(tool,name) : "genus"}]
            set _log_file "$_work_dir/${node_name}.log"
            set _module_cmd ""
            set _tool_shell "genus"
            catch {
