#!/usr/bin/env tclsh
# CBFlow SYNTH MMMC_SETUP Subnode Handler - Synopsys PT
...
set cmd_file "$::env(FLOW_DIR)/cmds/SYNTH/synopsys/fc/$_tool_ver/mmmc_setup_pt.tcl"
set test_mode false
...
switch $subnode_name {
    ...
    "resolve_scenarios" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists synth(tool,name)] ? $synth(tool,name) : "fc"}]
            ...
            puts "INFO: $stage_name resolve_scenarios completed \[TEST MODE\]"
        } else {
    ...
    "validate" {
        ...
        # Verify active_scenarios.tcl was generated
        set scenarios_file "$run_dir/work/$::flow_type/$node_name/run/active_scenarios.tcl"
        if {!$test_mode && ![file exists $scenarios_file]} {
