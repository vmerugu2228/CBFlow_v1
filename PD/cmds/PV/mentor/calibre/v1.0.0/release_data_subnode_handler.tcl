#!/usr/bin/env tclsh
# ═══════════════════════════════════════════════════════════════════════════════
# CBFlow PV RELEASE_DATA Subnode Handler
...
set _calibre_ver [expr {[info exists ::env(CALIBRE_VERSION)] ? $::env(CALIBRE_VERSION) : {v1.0.0}}]
set cmd_file "$::env(FLOW_DIR)/cmds/PV/mentor/calibre/$_calibre_ver/release_data_calibre.tcl"
...
switch $subnode_name {
    ...
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists pv(tool,name)] ? $pv(tool,name) : "calibre"}]
            set _log_file "$_work_dir/${node_name}.log"
            set _module_cmd ""
            set _tool_shell "calibre"
            catch {
