#!/usr/bin/env tclsh
# CBFlow FP EXPORT_DATA Subnode Handler - Synopsys FC
...
set cmd_file "$::env(FLOW_DIR)/cmds/FP/synopsys/fc/$_tool_ver/export_data_fc.tcl"
set test_mode false
...
switch $subnode_name {
    ...
    "run" {
        ...
        if {$test_mode} {
            ...
            set _tool_name [expr {[info exists fp(tool,name)] ? $fp(tool,name) : "fc"}]
            ...
            # Create dummy output files where real FC writes: results/fp/
            set _dn [expr {[info exists flow(design_name)] ? $flow(design_name) : "design"}]
            set _res "$run_dir/results/fp"
            file mkdir "$_res/def" "$_res/db"
            set _f [open "$_res/def/${_dn}.def" "w"]; puts $_f "// DEF floorplan \[TEST MODE\]"; close $_f
