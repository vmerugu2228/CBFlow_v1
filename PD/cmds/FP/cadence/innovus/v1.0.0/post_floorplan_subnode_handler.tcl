#!/usr/bin/env tclsh
# ═══════════════════════════════════════════════════════════════════════════════
# CBFlow PNR POST_FLOORPLAN Subnode Handler
# Description: Handles post_floorplan stage subnodes (setup, run, validate, finish)
...
# Load flow and FP configuration
...
set fp_config "$::env(CONFIG_ROOT)/flow/$::env(FLOW_CONFIG_VERSION)/node_configs/FP_config.tcl"
if {[file exists $fp_config]} { source $fp_config }

# Also load PNR config for shared Innovus tool settings
...
set cmd_file "$::env(FLOW_DIR)/cmds/PNR/cadence/innovus/[expr {[info exists ::env(INNOVUS_VERSION)] ? $::env(INNOVUS_VERSION) : "v1.0.0"}]/post_floorplan_handler.tcl"
