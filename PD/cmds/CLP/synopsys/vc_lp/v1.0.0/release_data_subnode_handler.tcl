#!/usr/bin/env tclsh
# CBFlow CLP RELEASE_DATA Subnode Handler - Synopsys VC_LP
...
set _tool_ver [expr {[info exists ::env(VC_LP_VERSION)] ? $::env(VC_LP_VERSION) : {v1.0.0}}]
set cmd_file "$::env(FLOW_DIR)/cmds/CLP/synopsys/vc_lp/$_tool_ver/release_data_vc_lp.tcl"
set test_mode false
    ...
    "setup" {
        ...
        if {[info exists ::env(GENERATION_VERSION)] && $::env(GENERATION_VERSION) ne ""} {
            ...
            if {[file exists $gen_script]} {
                puts "INFO: Generating setup files..."
                catch {exec tclsh $gen_script $::flow_type $node_name ${node_name}_default $run_dir}
            }
        }
